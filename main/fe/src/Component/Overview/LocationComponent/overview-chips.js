...
const OverviewChips = ({ client, locations, setLocationFilter }) => {
  ...
  useEffect(() => {
    ...
    locations &&
      locations.map((location) => {
        if (!chips.includes(location.client_loc_data.client_loc_country)) {
          chips.push(location.client_loc_data.client_loc_country);
        }
      });
    setChips(["All", ...chips]);
  }, [locations]);
